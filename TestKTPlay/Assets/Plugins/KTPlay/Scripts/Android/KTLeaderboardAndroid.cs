...
public class KTLeaderboardAndroid : MonoBehaviour
{
	...
	public static void GameLeaderboard(string leaderboardId,int startIndex,int count,MonoBehaviour obj,KTPlayLeaderboard.Callback callbackMethod)
	{
		if(obj!=null && callbackMethod!=null)
		{
			GameObject gameObj = obj.gameObject;
			if(gameObj != null && callbackMethod != null)
			{
				...
				{
#if UNITY_ANDROID
					KTPlayAndroid.joKTPlayAdapter.CallStatic("gameLeaderboard", leaderboardId, startIndex, count, gameObj.name, methodName);
#endif
				}
			}
		}
